...
void setup_endpoint_list() {
    ...
    // whose length is an exact multiple of max packet size (64 here)
    // -- confirmed via tinyusb's actual ChipIdea driver source and a
    // real Linux kernel stable-tree patch for this exact hardware
    // family ("usb: chipidea: udc: Disable auto ZLP generation").
    // We've never touched this bit, so hardware has been silently
    // auto-appending a ZLP after every 64-byte-multiple transfer this
    // entire session -- including READ(10)'s 512-byte sector data --
    // regardless of anything our own software did or didn't send.
    // Setting ZLT=1 disables this; MSC's host driver always knows the
    // exact expected length in advance and never wants this extra
    // packet.
    #define QH_ZLT (1u << 29)
    ep_list[0].config = (0x40 << 16) | (1 << 15) | QH_ZLT;
    ep_list[1].config = (0x40 << 16) | QH_ZLT;
    ep_list[2].config = (0x40 << 16) | QH_ZLT;
    ep_list[3].config = (0x40 << 16) | QH_ZLT;
...
if (ENDPTSETUPSTAT & 0x00000001) {
    ...
    if (bRequest == 0x01) { // CLEAR_FEATURE
        ...
        if (config_val > 0) {
            ...
            // if (g_usb_setup_callback) g_usb_setup_callback();
