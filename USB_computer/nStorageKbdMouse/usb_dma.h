...
static uint8_t ep_list_raw[sizeof(DeviceQueueHead) * 8 + EP_LIST_ALIGN];
...
static uint8_t dtd_bulk_tx_raw[sizeof(DeviceTransferDescriptor) + DTD_ALIGN];
static uint8_t dtd_bulk_rx_raw[sizeof(DeviceTransferDescriptor) + DTD_ALIGN];
...
#define COMBO_EP_LIST_ENTRIES 8

static uint8_t rx_buffer_raw[BULK_OUT_BUFFER_SIZE + DTD_ALIGN];

static DeviceQueueHead *ep_list;
static DeviceTransferDescriptor *dtd_tx;
static DeviceTransferDescriptor *dtd_rx;
static DeviceTransferDescriptor *dtd_bulk_tx;
static DeviceTransferDescriptor *dtd_bulk_rx;
...
static const uint8_t device_descriptor[18] = {
    ...
    0x09, 0x12, 0x03, 0x00, 0x00, 0x01, 0x01, 0x02,
    0x03, 0x01
};

// Composite device: interface 0 = MSC (bulk EP1, both directions),
// interface 1 = HID keyboard (interrupt EP2 IN), interface 2 = HID
// mouse (interrupt EP3 IN). Uses the hardware's full, documented
// 4-endpoint budget (hackspire: DCCPARAMS default 0x184 = host-
// capable, device-capable, 4 endpoints) -- EP0 control + these three.
static const uint8_t config_descriptor[82] = {
    // Configuration descriptor
    0x09, 0x02, 0x52, 0x00, 0x03, 0x01, 0x00, 0x80, 0x32,
    // Interface 0: MSC
...
void save_native_usb_state(void) {
    ...
    native_usb_state.endptctrl2  = ENDPTCTRL2;
    native_usb_state.endptctrl3  = ENDPTCTRL3;
...
void restore_native_usb_state(void) {
    ...
    ENDPTCTRL2        = native_usb_state.endptctrl2;
    ENDPTCTRL3        = native_usb_state.endptctrl3;
...
void init_aligned_pointers(void) {
    ...
    dtd_bulk_tx  = (DeviceTransferDescriptor *)align_up((uint32_t)dtd_bulk_tx_raw, DTD_ALIGN);
    dtd_bulk_rx  = (DeviceTransferDescriptor *)align_up((uint32_t)dtd_bulk_rx_raw, DTD_ALIGN);
...
void setup_endpoint_list() {
    memset(ep_list, 0, sizeof(DeviceQueueHead) * COMBO_EP_LIST_ENTRIES);
    ...
    // via tinyusb's actual ChipIdea driver source and a real Linux
    // kernel stable-tree patch for this exact hardware family ("usb:
    // chipidea: udc: Disable auto ZLP generation"). Setting ZLT=1
    // disables this; MSC's host driver always knows the exact expected
    // length in advance and never wants this extra packet, and HID
    // reports are fixed-size and never want it either.
    #define QH_ZLT (1u << 29)
    ep_list[0].config = (0x40 << 16) | (1 << 15) | QH_ZLT; // EP0 OUT
    ep_list[1].config = (0x40 << 16) | QH_ZLT;              // EP0 IN
    ep_list[2].config = (0x40 << 16) | QH_ZLT;              // EP1 OUT (MSC bulk)
    ep_list[3].config = (0x40 << 16) | QH_ZLT;              // EP1 IN (MSC bulk)
    ep_list[4].config = (0x08 << 16) | QH_ZLT;              // EP2 OUT (unused)
    ep_list[5].config = (0x08 << 16) | QH_ZLT;              // EP2 IN (HID keyboard reports)
    ep_list[6].config = (0x08 << 16) | QH_ZLT;              // EP3 OUT (unused)
    ep_list[7].config = (0x08 << 16) | QH_ZLT;              // EP3 IN (HID mouse reports)

    for (int i = 0; i < COMBO_EP_LIST_ENTRIES; i++) {
        ep_list[i].current_dtd = 0x00000001;
    }

    drain_write_buffer();
    clean_dcache_range(ep_list, sizeof(DeviceQueueHead) * COMBO_EP_LIST_ENTRIES);

    ENDPOINTLISTADDR = (uint32_t)ep_list;

    // EXPERIMENT: the real ChipIdea Linux driver enables these interrupt
    // bits (hw_device_state(), UI|UEI|PCI|URI) right alongside writing
    // ENDPOINTLISTADDR, even though it's interrupt-driven overall --
    // we've left USBINTR at 0 this entire session since we poll instead
    // of using real interrupts. We're not adding an actual interrupt
    // handler here, still fully polled -- this only tests whether
    // merely having these status-latching bits enabled changes
    // hardware's internal behavior, since some undocumented silicon
    // quirks gate internal state transitions on enabled status bits,
    // not just on whether the IRQ line actually gets asserted.
...
void send_usb_message(const char* msg, uint32_t length) {
    int flush_ok;
    ENDPTFLUSH = (1u << 17);
    WAIT_CLEAR_TIMEOUT(ENDPTFLUSH, (1u << 17), 2000000, flush_ok);
    if (!flush_ok) dbg_print("     send_test_message: pre-flush TIMEOUT");

    USBSTS = USBSTS;

    dtd_bulk_tx->next_dtd = 0x00000001;
    dtd_bulk_tx->token = (length << 16) | (1 << 15) | 0x80;
    dtd_bulk_tx->buffer[0] = (uint32_t)msg;
    {
        uint32_t page_base = (uint32_t)msg & ~0xFFFu;
        for (int p = 1; p < 5; p++) {
    ...
    if (!ok) dbg_print("     send_test_message: TIMEOUT");

    int ok_complete;
    WAIT_SET_TIMEOUT(ENDPTCOMPLETE, (1 << 17), 5000000, ok_complete);
    if (!ok_complete) {
        dbg_print("     send_test_message: complete TIMEOUT");
        print_ep1_in_state("     post-timeout EP1 IN state:");
    } else {
        ENDPTCOMPLETE = (1 << 17);
        dbg_print("     send_usb_message: SUCCESS");
    }
}

void arm_bulk_out() {
    print_ep1_out_state("     pre-arm_bulk_out:");

    int flush_ok;
    ENDPTFLUSH = (1u << 1);
    WAIT_CLEAR_TIMEOUT(ENDPTFLUSH, (1u << 1), 2000000, flush_ok);
    if (!flush_ok) dbg_print("     arm_bulk_out: pre-flush TIMEOUT");

    dtd_bulk_rx->next_dtd = 0x00000001;
    dtd_bulk_rx->token = (BULK_OUT_BUFFER_SIZE << 16) | (1 << 15) | 0x80;
    dtd_bulk_rx->buffer[0] = (uint32_t)rx_buffer;
    {
        uint32_t page_base = (uint32_t)rx_buffer & ~0xFFFu;
        for (int p = 1; p < 5; p++) {
            dtd_bulk_rx->buffer[p] = page_base + ((uint32_t)p * 0x1000u);
        }
    }

    ep_list[2].td.next_dtd = (uint32_t)dtd_bulk_rx;
    ep_list[2].td.token &= ~0xC0;

    clean_dcache_range(dtd_bulk_rx, sizeof(*dtd_bulk_rx));
    ...
    if (!ok) {
        dbg_print("     arm_bulk_out: prime TIMEOUT");
    }
}

// Sends one keyboard report (8 bytes) via EP2 IN (index 5 in ep_list,
// prime/complete bit 18). Blocks until hardware confirms completion.
void send_keyboard_report(const uint8_t *report, uint32_t length) {
    ...
    ep_list[5].td.next_dtd = (uint32_t)dtd_kbd_tx;
    ...
    int ok2;
    WAIT_CLEAR_TIMEOUT(ENDPTPRIME, (1 << 18), 5000000, ok2);
    if (!ok2) dbg_print("     send_keyboard_report: prime TIMEOUT");
...
// Sends one mouse report (4 bytes) via EP3 IN (index 7 in ep_list,
// prime/complete bit 19).
void send_mouse_report(const uint8_t *report, uint32_t length) {
    ...
    ep_list[7].td.next_dtd = (uint32_t)dtd_mouse_tx;
    ep_list[7].td.token &= ~0xC0;

    clean_dcache_range(dtd_mouse_tx, sizeof(*dtd_mouse_tx));
    clean_dcache_range(&ep_list[7].td, sizeof(ep_list[7].td));
    clean_dcache_range(report, length);
    drain_write_buffer();
    ...
    int ok2;
    WAIT_CLEAR_TIMEOUT(ENDPTPRIME, (1 << 19), 5000000, ok2);
    if (!ok2) dbg_print("     send_mouse_report: prime TIMEOUT");

    int ok_complete;
    WAIT_SET_TIMEOUT(ENDPTCOMPLETE, (1 << 19), 5000000, ok_complete);
    if (!ok_complete) {
...
if (ENDPTSETUPSTAT & 0x00000001) {
    ...
    // bits 5-6 of bmRequestType: 00=Standard, 01=Class, 10=Vendor.
    // Needed to disambiguate bRequest==0x01, which means CLEAR_FEATURE
    // for a standard request but GET_REPORT for HID's class-specific
    // requests -- without this check, GET_REPORT would be silently
    // swallowed as a no-op CLEAR_FEATURE instead of reaching the HID
    // handler.
    int is_standard_request = ((bmRequestType & 0x60) == 0x00);
    ...
    if (bRequest == 0x01 && is_standard_request) { // CLEAR_FEATURE
        if (wValue == 0x00) {
            ...
            if (ep_num == 1) {
                if (is_in) {
                    ...
                    if (g_usb_setup_callback) {
                        dbg_print("     CLEAR_FEATURE: invoking setup callback");
                        g_usb_setup_callback();
                    } else {
                        dbg_print("     CLEAR_FEATURE: no setup callback registered!");
                    }
                } else {
                    ENDPTCTRL1 &= ~ENDPTCTRL_RXS;
                    ENDPTCTRL1 |= ENDPTCTRL_RXR;
                    ep_list[2].td.token &= ~0xC0;
                    clean_dcache_range(&ep_list[2].td, sizeof(ep_list[2].td));
                    arm_bulk_out();
                }
                dbg_print("     CLEAR_FEATURE(ENDPOINT_HALT) applied to EP1");
            } else if (is_in && (ep_num == 2 || ep_num == 3)) {
                // HID endpoints -- IN only, no OUT direction to re-arm.
                if (ep_num == 2) {
                    ...
                    ENDPTCTRL3 &= ~ENDPTCTRL_TXS;
                    ENDPTCTRL3 |= ENDPTCTRL_TXR;
                    ep_list[7].td.token &= ~0xC0;
                    clean_dcache_range(&ep_list[7].td, sizeof(ep_list[7].td));
                }
                dbg_print("     CLEAR_FEATURE(ENDPOINT_HALT) applied to HID endpoint");
            }
        }
        ...
        if (config_val > 0) {
            ...
            // HID endpoints -- TX (IN) only, no OUT direction needed.
            ENDPTCTRL2 = ENDPTCTRL_TXE | ENDPTCTRL_TXR | ENDPTCTRL_TXT_INTERRUPT;
            ENDPTCTRL3 = ENDPTCTRL_TXE | ENDPTCTRL_TXR | ENDPTCTRL_TXT_INTERRUPT;
...
void service_bulk_endpoints() {
    ...
    if (complete & (1 << 1)) {
        ...
        invalidate_dcache_line(dtd_bulk_rx);
        invalidate_dcache_line(((uint8_t*)dtd_bulk_rx) + 32);
        drain_write_buffer();

        uint32_t remaining = (dtd_bulk_rx->token >> 16) & 0x7FFF;
        uint32_t received = BULK_OUT_BUFFER_SIZE - remaining;

        if (received > 0 && received <= BULK_OUT_BUFFER_SIZE) {
            invalidate_dcache_range(rx_buffer, BULK_OUT_BUFFER_SIZE);
            drain_write_buffer();
            ...
            char textbuf[BULK_OUT_BUFFER_SIZE + 4];
            uint32_t n = received;
            if (n > BULK_OUT_BUFFER_SIZE - 1) n = BULK_OUT_BUFFER_SIZE - 1;
            ...
            if (g_usb_rx_callback) {
                g_usb_rx_callback((const uint8_t *)rx_buffer, received);
            }
        }
