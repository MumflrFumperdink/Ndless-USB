...
/* ==================================================================
 * String descriptors (UTF-16LE) -- the single, unified table for the
 * whole composite device (MSC + HID keyboard + HID mouse all share
 * one set, indexed from the device descriptor's iManufacturer/
 * iProduct/iSerialNumber -- not one table per subsystem).
 * ================================================================== */
...
static const uint8_t hid_string_desc_product[60] = {
    60, 0x03,
    'N',0,'s',0,'p',0,'i',0,'r',0,'e',0,' ',0,'S',0,'t',0,'o',0,'r',0,'a',0,'g',0,'e',0,'+',0,'K',0,'e',0,'y',0,'b',0,'o',0,'a',0,'r',0,'d',0,'+',0,'M',0,'o',0,'u',0,'s',0,'e',0
};
...
void hid_on_unhandled_control(uint8_t bRequest, uint16_t wValue, uint16_t wIndex, uint16_t wLength) {
    if (bRequest == 0x06) { // GET_DESCRIPTOR
        ...
        if (desc_type == 0x21) { // HID Descriptor itself -- pull straight from the config descriptor, same bytes
            if (wIndex == 1) {
                transmit_ep0_data(&config_descriptor[41], 9, wLength);
            } else if (wIndex == 2) {
                transmit_ep0_data(&config_descriptor[66], 9, wLength);
            }
            return;
        }
        return;
